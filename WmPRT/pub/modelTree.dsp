...
function createTree(modelList, modelTreeList, models, ModelID, ModelVersion)
{
	...
	for (var lp=0; lp<Nodes.length-1; lp++)
	{
		...
		if (modelTree != null)
		{
			...
			if (lp > 0)
			{
				modelTree = new WebFXTreeItem(Nodes[lp], "list-all-process.dsp?mid=" + element + "/" + "?_ALL_",
					null, webFXTreeConfig.folderIcon, webFXTreeConfig.openFolderIcon);
			}
			else
			{
				modelTree = new WebFXTreeItem(Nodes[lp], "list-all-process.dsp?mid=" + element + "/" + "?_ALL_",
					null, webFXTreeConfig.BPMfolder, webFXTreeConfig.BPMfolder);
			}	
			modelTree.target = "propertyFrame";
	...
	if (model == null)
	{
		model = new WebFXTreeItem(Nodes[Nodes.length-1], "list-all-process.dsp?mid=" + ModelID + "?_ALL_",
			null, webFXTreeConfig.BPMmodel, webFXTreeConfig.BPMmodel);
		model.target = "propertyFrame";
...
var models = new WebFXTree("Processes", "list-all-process.dsp?mid=_ALL_?_ALL_",	null,
	webFXTreeConfig.BPMprocess, webFXTreeConfig.BPMprocess);
